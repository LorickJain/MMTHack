...
if departure_city or arrival_city not in cities:
	print("Please enter an appropriate city")
else:
	dep_code = cities['departure_city']
	arr_code = cities['arrival_city']
if len(departure_date) or len(arrival_date) != 10 :
	print("Invalid date, please enter a valid date!") 
url = "https://flights.makemytrip.com/makemytrip/search/R/R/E/1/0/0/S/V0/" + dep_code + "_" + arr_code + "_" + departure_date + "," + arrival_date + "?intid=DF_LP_Widget_Search_"+ departure_city.title() + "_" + arrival_city.title()
connection = request.urlopen(url)
source = connection.read()
soup = BeautifulSoup(source, "html.parser")

for i in soup.findAll('span', {'class': 'INR'}):
	ticket_prices.append(i.contents)
actual_price = ticket_prices[0]
print(actual_price);print(datetime.datetime.now().strftime('%H:%M:%S'))#It will write output in the file , then do cat * > all_prices.txt, means after you run it multiple times and for every half an hour it creates a new file, concatenate all these.
